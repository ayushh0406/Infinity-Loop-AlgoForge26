...
import {
  registerBorrower, loginUser,
  initiatePanVerification, verifyPanOtp,
  initiateAadhaarVerification, verifyAadhaarOtp,
} from '../services/apiService';

/**
 * Borrower Login / KYC Flow — identical structure to LenderLogin.jsx
...
const demoAccounts = [
  { email: 'student@example.com', label: 'Student Demo' },
  { email: 'gig@example.com',     label: 'Gig Worker Demo' },
  { email: 'salaried@example.com',label: 'Salaried Demo' },
];
...
export default function Login() {
  ...
  const [panName, setPanName] = useState('');   // Name as per PAN
  const [panDob, setPanDob]   = useState('');   // DOB: DD/MM/YYYY
  ...
  const handleDemoLogin = async (email) => {
    ...
    try {
      ...
      login(data);
      addToast({ message: 'Demo login successful!', type: 'success' });
      setTimeout(() => navigate('/dashboard'), 500);
    } catch (err) {
      addToast({ message: err.message || 'Demo login failed', type: 'error' });
  ...
  const handleAuth = async (e) => {
    ...
    try {
      let data;
      if (isSignUp) {
        if (!form.name) { addToast({ message: 'Name is required', type: 'error' }); setLoading(false); return; }
        await registerBorrower({ name: form.name, email: form.email, password: form.password });
        ...
        if (data.user?.role === 'lender') {
          addToast({ message: 'This is a lender account. Please use the Lender Portal.', type: 'error' });
      ...
      setAlreadyKyc(kyc);
      if (kyc.pan && kyc.aadhaar) { setStep(3); } else { setStep(1); }
    } catch (err) {
  ...
  const handleVerifyPan = async () => {
    ...
    try {
      ...
      // Auto-advance: mark verified in backend then move to Aadhaar
  ...
  const handleSendAadhaar = async () => {
    ...
    try {
      ...
      setAadhaarRef(res.referenceId); setAadhaarSent(true);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#020817] via-[#0f1b3a] to-[#020817] flex items-center justify-center p-4 relative overflow-hidden">
      {/* Background orbs — same as LenderLogin.jsx */}
      ...
      >
        <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl p-8 shadow-2xl">
          ...
          <div className="text-center mb-6">
            ...
            <p className="text-gray-400 text-sm">Borrower Portal — KYC Verification</p>
          </div>
              ...
              <motion.div key="step0" variants={slideVariants} initial="initial" animate="animate" exit="exit" transition={{ duration: 0.3 }}>
                <form onSubmit={handleAuth} className="space-y-4 mb-4">
                  ...
                  >
                    {loading
                      ...
                      : <>{isSignUp ? 'Create Account & Continue' : 'Sign In & Continue'} <ArrowRight className="w-4 h-4" /></>}
                  </button>
                </form>
                ...
                <div className="bg-white/5 border border-white/10 rounded-lg p-4">
                  <p className="text-xs text-gray-400">
                    <strong>Note:</strong> After login, you'll complete PAN + Aadhaar KYC to access the borrower dashboard.
                  </p>
                </div>
              </motion.div>
            )}
            ...
            {step === 1 && (
              <motion.div key="step1" variants={slideVariants} initial="initial" animate="animate" exit="exit" transition={{ duration: 0.3 }}>
                <div className="mb-6">
                  ...
                  <p className="text-gray-400 text-sm">Required for borrower compliance (RBI guidelines)</p>
                </div>
            ...
            {step === 2 && (
              <motion.div key="step2" variants={slideVariants} initial="initial" animate="animate" exit="exit" transition={{ duration: 0.3 }}>
                <div className="mb-6">
                  ...
                  <p className="text-gray-400 text-sm">Final KYC step to activate your borrower account</p>
                </div>
              ...
              <motion.div key="step3" variants={slideVariants} initial="initial" animate="animate" exit="exit" transition={{ duration: 0.3 }}>
                <div className="text-center py-4">
                  ...
                  <p className="text-gray-400 text-sm mb-6">
                    Your identity has been verified. You can now apply for loans on TrustPool.
                  </p>

                  <div className="grid grid-cols-3 gap-3 mb-6">
                    {[
                      { label: 'Starting Wallet', value: '₹10,000' },
                      { label: 'Max Loan',         value: '₹1L+' },
                      { label: 'AI Scoring',       value: 'Live' },
                    ].map(s => (
